...
def train(
    model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler,
):
    # train
    model.train()
    ...
    for i, (image, _, text) in enumerate(
...
def get_train_arrows():
    ...
    cc_files = [f"arrow/cc/cc_{i}.arrow" for i in range(31)]
    sbu_files = [f"arrow/sbu/sbu_{i}.arrow" for i in range(9)]
    vg_files = ["arrow/vg/vg_albef.arrow"]

    train_arrows = coco_files + cc_files + sbu_files + vg_files
    return train_arrows
